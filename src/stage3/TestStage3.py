...
class TestStage3(unittest.TestCase):
    def run_stage3(self, user_inputs):
        """
        Uruchamia stage3.py z listą wejść jako symulacją interakcji z użytkownikiem.
        """
    ...
    def test_help_command(self):
        output = self.run_stage3(["!help", "!done"])
    ...
    def test_new_line_formatter(self):
        output = self.run_stage3(["plain", "Line1", "new-line", "plain", "Line2", "!done"])
        self.assertIn("Line1\nLine2", output)  # nowa linia powinna rozdzielać

    def test_combination_output(self):
        output = self.run_stage3([
            "header", "1", "Title",
            "plain", " text ",
            "bold", "bold",
            "italic", "italic",
